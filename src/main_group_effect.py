...
from utils import (
    obtain_f_x_a_from_f_x_b, 
    obtain_pi_phi_2nd, 
    obtain_pi_phi_2nd_lack,
    obtain_meta_info_hist
)
...
def main(cfg: DictConfig) -> None:
    ...
    group_effect_coeff_list = cfg.setting.vary.group_effect_coeff_list
    use_wandb = cfg.setting.base_setting.use_wandb


    print("===== Configuration =====")
    print(group_effect_coeff_list)
    ...
    dataset = ExtremeBanditDataset(
        ...
        group_effect_coeff = group_effect_coeff_list[0],
        cost_effect_coeff = cfg.setting.dataset.cost_effect_coeff,
    ...
    result_df = DataFrame()
    meta_info_df_list = []
    for group_effect_coeff in group_effect_coeff_list:

        dataset.group_effect_coeff = group_effect_coeff
        if group_effect_coeff == group_effect_coeff_list[0]:
        ...
        m_ratio_list = []
        for _ in tqdm(range(num_runs), desc=f"group_effect_coeff={group_effect_coeff}..."):
            ...
            if train_bandit_data["n_m_max"] != test_bandit_data["n_m_max"]:
                print("n_m_max is different between train and test!")
                continue

            if group_effect_coeff != 0.0:
                use_wandb = False

            if use_wandb:
                wandb.init(
                    project=cfg.setting.wandb.project_name, 
                    name=f"g_coef{group_effect_coeff}_seed{_}",
                    group="IPS"
                )
            ...
            pi_phi_2nd = obtain_pi_phi_2nd(f_hat_x_b=f_hat_x_b, dataset_train=train_bandit_data, independent_reward_type=cfg.setting.dataset.independent_reward_type)
            ...
            pi_phi_2nd_test = obtain_pi_phi_2nd(f_hat_x_b=f_hat_x_b_test, dataset_train=test_bandit_data, independent_reward_type=cfg.setting.dataset.independent_reward_type)
            ours.fit(
            ...
            meta_info_hist = obtain_meta_info_hist(
                ...
                meta_info_candidate=cfg.setting.dataset.group_effect_coeff_candidate,
                meta_info_name="group_effect_coeff",
            )
            m_ratio_list.append({"group_effect_coeff": group_effect_coeff, "meta_info_hist": meta_info_hist})
            if use_wandb:
                wandb.finish()
        ...
        result_df.to_csv(f"{save_path}group_effect_coeff.csv")
        meta_info_df = pd.concat(meta_info_df_list).reset_index(level=0)
        meta_info_df.to_csv(f"{save_path}meta_info_group_effect_coeff.csv")
        plot(
            result_df=result_df,
            varying_name="group_effect_coeff",
            varying_list=group_effect_coeff_list,
            axis_name="Group Effect Coefficient",
            log_scale=False,
            save_path=save_path,
        )

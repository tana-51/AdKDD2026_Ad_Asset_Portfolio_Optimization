...
def main(cfg: DictConfig) -> None:
    ...
    for num_sample in num_sample_list:
        ...
        group_effect_coeff_candidate = np.linspace(0.0, 1.0, num=num_sample)
        cost_effect_coeff_candidate = np.linspace(0.0, 0.1, num=num_sample)
        ...
        dataset = ExtremeBanditDataset(
            ...
            group_effect_coeff_candidate = group_effect_coeff_candidate,
            cost_effect_coeff_candidate = cost_effect_coeff_candidate,
        ...
        m_ratio_list = []
        for _ in tqdm(range(num_runs), desc=f"num_sample={num_sample}..."):
            ...
            if use_wandb:
                wandb.init(
                    ...
                    name=f"num_sample_{num_sample}_seed{_}",
                    group="IPS"
                )
            ...
            m_ratio_list.append({"num_sample": num_sample, "meta_info_hist": meta_info_hist})
            if use_wandb:
                wandb.finish()
        ...
        result_df["num_sample"] = num_sample
        ...
        result_df.to_csv(f"{save_path}num_sample.csv")
        meta_info_df = pd.concat(meta_info_df_list).reset_index(level=0)
        meta_info_df.to_csv(f"{save_path}meta_info_num_sample.csv")
        plot(
            result_df=result_df,
            varying_name="num_sample",
            varying_list=num_sample_list,

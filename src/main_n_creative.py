"""
vary n_unique_action_threshold
"""
...
from utils import obtain_f_x_a_from_f_x_b, obtain_pi_phi_2nd
...
def main(cfg: DictConfig) -> None:
    ...
    n_unique_action_threshold_list = cfg.setting.vary.n_unique_action_threshold_list
    use_wandb = cfg.setting.base_setting.use_wandb

    print("===== Configuration =====")
    print(n_unique_action_threshold_list)

    result_df_list = []
    result_df = DataFrame()
    for n_unique_action_threshold in n_unique_action_threshold_list:

        dataset = ExtremeBanditDataset(
            dim_context = cfg.setting.dataset.dim_context,
            n_unique_action_threshold = n_unique_action_threshold,
        ...
        for _ in tqdm(range(num_runs), desc=f"n_unique_action_threshold={n_unique_action_threshold}..."):
            ...
            if use_wandb:
                wandb.init(
                    ...
                    name=f"n_creative{n_unique_action_threshold}_seed{_}",
                    group="IPS"
                )
        ...
        result_df["n_unique_action_threshold"] = n_unique_action_threshold
        ...
        result_df.to_csv(f"{save_path}n_unique_action_threshold.csv")
        plot(
            result_df=result_df,
            varying_name="n_unique_action_threshold",
            varying_list=n_unique_action_threshold_list,
            axis_name="Number of Creatives",
            log_scale=False,
            save_path=save_path,
        )

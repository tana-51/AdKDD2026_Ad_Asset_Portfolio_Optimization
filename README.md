...
## Abstract
In online advertising, it is crucial to identify optimal ad assets for diverse users to maximize platform revenue. Many algorithms have been developed for selecting personalized ad assets, and have been applied in industrial systems. However, to fully leverage the performance of these algorithms, advertisers are required to submit an appropriate set of ad assets, which we call an \textit{ad asset portfolio}, to the platforms. This problem, which we call \textit{ad asset portfolio optimization (Ad-POP)}, is a crucial issue for online advertising, but it remains underexplored in the existing literature.
